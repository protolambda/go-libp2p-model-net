...
func newLink(env Environment, opts LinkOptions, n1 *peernet, n2 *peernet) *link {
	l := &link{
		env:         env,
		nets:        [2]*peernet{n1, n2},
		opts:        opts,
		ratelimiter: NewRateLimiter(env, opts.Bandwidth)}
	return l
}
...
func (l *link) newStreamPair() (*stream, *stream) {
	...
	sa := NewStream(l.env, wa, ra, network.DirOutbound)
	sb := NewStream(l.env, wb, rb, network.DirInbound)
	return sa, sb
}
